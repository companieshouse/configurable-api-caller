...
resource "aws_cloudwatch_event_rule" "call_api_caller_lambda_account_validator_cleanup_submissions" {
  ...
  description         = var.deploy_to == "live" ? "Cloudwatch event to call ${aws_lambda_function.configurable_api_lambda.function_name} lambda at 2am everyday" : "Cloudwatch event to call ${aws_lambda_function.configurable_api_lambda.function_name} lambda at 7pm everyday"
  schedule_expression = var.deploy_to == "live" ? "cron(0 2 * * ? *)" : "cron(0 19 * * ? *)"
}
